...
public class AbstractRedis {
	...
	protected double getScore(String key, boolean asc) {
		...
		Integer count = unitdb.queryForInt(sql, key);
	...
	public RedisEntity getBean(String key, String member) {
		...
		RedisEntity bean = unitdb.query(sql, RedisEntity.class, key, member);
		return bean;
	}

	protected String getByField(String key, String field) {
		String sql = "select value from " + TABLE + " where `key`=? and score=?;";
		String value = unitdb.queryForString(sql, key, field);
	...
	protected synchronized boolean delete(String key, String member) {
		...
		int count = unitdb.update(sql, key, member);
		return count > 0;
	}
	...
	protected synchronized boolean deleteByField(String key, String field) {
		...
		int count = unitdb.update(sql, key, field);
		return count > 0;
	}
	...
	protected synchronized boolean insert(String key, String field, String value, Date expiry) {
		...
		int count = unitdb.update(sql, key, field, value, expiry);
		return count > 0;
	}
